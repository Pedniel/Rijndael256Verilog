module key_expansion_dual(
                          // 0 = 192-bit, 1 = 256-bit                          
                          input wire         key_mode, 
                          input wire [255:0] key_in, 
                          output [3839:0]    round_keys_flat
                          );
   ...
   always @(*) begin
      if (key_mode == 0) begin // 192bit key
         for (i = 0; i < 6; i = i + 1) begin
            word[i] = key_in[191 - i*32 -: 32];
         end
         
         for (i = 6; i < 120; i = i + 1) begin // Generate remaining 114 words
            temp = word[i-1];
            
            if (i % 6 == 0) begin
               temp = SubWord(RotWord(temp)) ^ Rcon(i/6);
            end
            
            word[i] = word[i-6] ^ temp;
         end
      end else begin // 256bit key            
   ...
   function [31:0] RotWord;  // Rotate a 32bit word left by 1 byte
      input [31:0] w;
      begin
